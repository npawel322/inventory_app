...
class AssetSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Asset
        fields = [
            "id", "category", "category_name", "name", "serial_number",
            "asset_tag", "status", "purchase_date", "notes"
        ]
...
class LoanSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            "id",
            "asset", "asset_label",
            "person", "desk", "office", "department",
            "loan_date", "due_date", "return_date",
            "issued_by",
            "target",
        ]
        read_only_fields = ["loan_date", "return_date", "target", "asset_label"]
    ...
    def validate(self, attrs):
        ...
        chosen = [(k, v) for (k, v) in targets if v]
        if len(chosen) != 1:
            raise serializers.ValidationError("Choose exactly one target: person OR desk OR office OR department.")

        # zapisujemy przycięty department
        if department:
    ...
    def create(self, validated_data):
        # zabezpieczenie: ustawiamy status assetu w transakcji
